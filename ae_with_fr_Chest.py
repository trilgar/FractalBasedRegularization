...
MODEL_NAME       = "fd_brain_tumor_l40_m005_r_010"

# Fractal regularization
LAMBDA_FD = 40
RC_RATE   = 0.1   # intermediate recon MSE weight inside reconstruction loss
...
class BrainTumorClsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_path = row["image"]
        # Більшість МРТ — grayscale, але модель очікує 3 канали: конвертуємо в RGB
...
class SemiSupervisedDataset(Dataset):
    """
    Повертає (labeled_x, labeled_y, unlabeled_x) з рівномірним семплінгом
    серед усіх розмічених прикладів (без p_min і будь-якого балансування).
    """
    ...
    def __len__(self):
        return self.unlabeled_size  # керує розміром батча

    def __getitem__(self, idx):
        li = np.random.randint(0, self.labeled_size)
...
class BrainTumorDataModule(LightningDataModule):
    def __init__(self, batch_size=BATCH_SIZE):
        ...
        self.ce_weights = None  # без ваг у CE
    ...
    def _scan_split(self, split_dir: str) -> pd.DataFrame:
        """
        Збирає усі шляхи виду split_dir/class_name/*.(jpg|png|jpeg)
        Повертає DataFrame: image (abs path), label (class_name), label_idx (int)
        """
        rows = []
        # якщо class_to_idx ще не відомий (перший прохід — по TRAIN_DIR), створимо з підпапок
        class_names = []
        ...
        # На тренуванні формуємо мапінг; на тесті використовуємо вже відомий
        ...
        for cls in self.class_to_idx.keys():
            ...
            if not os.path.isdir(cls_dir):
                # якщо у тесті відсутня якась клас-папка — пропускаємо
                continue
            files = []
            for e in exts:
    ...
    def setup(self, stage=None):
        global NUM_CLASSES
        NUM_CLASSES = self.num_classes  # синхронізуємо з глобальною змінною для моделі/втрат

        # Stratified train/val з TRAIN_DIR
        train_split = 0.9
        valid_split = 0.025
        ...
        # Рівномірний семплінг без балансування: використовуємо спрощений SemiSupervisedDataset
        # з сигнатурою SemiSupervisedDataset(labeled_ds, unlabeled_ds)
        self.semi_train = SemiSupervisedDataset(self.train_labeled, self.train_unlabeled)

        # Без класових ваг у CE (класи ~збалансовані)
        self.ce_weights = None

        print(f"[BrainTumor] Classes: {self.class_to_idx}")
        print(f"[BrainTumor] Labeled size: {len(self.train_labeled)}; Unlabeled size: {len(self.train_unlabeled)}")
...
class LitFractalAE(LightningModule):
    def __init__(self, num_classes, rc_rate=RC_RATE, lr=LR, lambda_fd=LAMBDA_FD, ce_weights=None):
        super().__init__()
...
def main():
    ...
    model = LitFractalAE(
        ...
        rc_rate=RC_RATE, lr=LR, lambda_fd=LAMBDA_FD,
